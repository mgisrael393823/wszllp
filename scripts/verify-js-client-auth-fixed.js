...
/**
 * Verify JavaScript Client Authentication - Fixed Version
 * This script properly checks for affected rows, not just errors
 */
...
async function testAuthentication() {
  console.log('🔍 Testing Supabase Client Authentication (Fixed)\n');
  ...
  if (user2CreateError) {
    ...
    if (!user2SignInError && testCase) {
      ...
      // Try to update user 1's case - FIXED VERSION
      console.log('\n   Testing UPDATE...');
      const { data: updateData, error: updateError, count } = await user2Client
        .from('cases')
      ...
      if (updateError) {
        console.log('   ✅ UPDATE blocked with error:', updateError.message);
      } else if (!updateData || updateData.length === 0) {
        console.log('   ✅ UPDATE blocked by RLS (0 rows affected)');
      } else {
        console.error('   ❌ SECURITY ISSUE: User 2 successfully updated User 1 case!');
        console.error('      Updated rows:', updateData.length);
      }
      
      // Try to delete user 1's case - FIXED VERSION
      console.log('\n   Testing DELETE...');
      ...
      if (deleteError) {
        console.log('   ✅ DELETE blocked with error:', deleteError.message);
      } else if (!deleteData || deleteData.length === 0) {
        console.log('   ✅ DELETE blocked by RLS (0 rows affected)');
      } else {
        console.error('   ❌ SECURITY ISSUE: User 2 successfully deleted User 1 case!');
        console.error('      Deleted rows:', deleteData.length);
      }
      
      // Verify the case still exists and is unchanged
      const { data: finalCheck, error: finalError } = await anonClient
        .from('cases')
        .select('status')
        .eq('id', testCase.id)
        .single();
      
      if (finalCheck && finalCheck.status === 'Test') {
        console.log('\n   ✅ Final verification: Case unchanged');
      } else if (finalCheck) {
        console.error(`   ❌ Case was modified! Status: ${finalCheck.status}`);
      } else {
        console.error('   ❌ Case was deleted!');
      }
    }
