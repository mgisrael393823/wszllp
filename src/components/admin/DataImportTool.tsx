...
const saveContactsToSupabase = async (contacts: any[]) => {
  console.log('Saving contacts to Supabase:', contacts);
...
const saveCasesToSupabase = async (cases: any[]) => {
  console.log('Saving cases to Supabase:', cases);
  ...
  try {
    ...
    const supabaseCases = cases.map(caseItem => ({
      ...
      status: caseItem.status || 'Intake',
      intakedate: caseItem.intakeDate || new Date().toISOString()
      // created_at, updated_at will be auto-generated by Supabase
    }));

    console.log('Transformed cases for Supabase:', supabaseCases);
    ...
    console.log('Cases saved/updated successfully:', data);
    return { success: true, saved: data?.length || 0, data };
  } catch (error) {
    console.error('Error saving cases to Supabase:', error);
    throw error;
  }
};
...
const saveHearingsToSupabase = async (hearings: any[]) => {
  console.log('Saving hearings to Supabase:', hearings);
  ...
  try {
    ...
    console.log('Transformed hearings for Supabase:', supabaseHearings);
    ...
    console.log('Hearings saved/updated successfully:', data);
    return { success: true, saved: data?.length || 0, data };
  } catch (error) {
    console.error('Error saving hearings to Supabase:', error);
    throw error;
  }
};
...
const saveDocumentsToSupabase = async (documents: any[]) => {
  console.log('Saving documents to Supabase:', documents);
  ...
  try {
    ...
    console.log('Transformed documents for Supabase:', supabaseDocuments);
    ...
    console.log('Documents saved/updated successfully:', data);
    return { success: true, saved: data?.length || 0, data };
  } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (enhancedCases.length > 0) {
        console.log('Saving cases to Supabase...');
        try {
          const supabaseResult = await saveCasesToSupabase(enhancedCases);
          console.log('Cases saved/updated in Supabase:', supabaseResult);
        } catch (error) {
          console.error('Failed to save cases to Supabase:', error);
          // Continue with local save even if Supabase fails
        }
      }

      if (entities.hearings.length > 0) {
        console.log('Saving hearings to Supabase...');
        try {
          const supabaseResult = await saveHearingsToSupabase(entities.hearings);
          console.log('Hearings saved/updated in Supabase:', supabaseResult);
        } catch (error) {
          console.error('Failed to save hearings to Supabase:', error);
          // Continue with local save even if Supabase fails
        }
      }
      ...
      if (entities.documents.length > 0) {
        ...
        try {
          ...
          console.log('Documents saved/updated in Supabase:', supabaseResult);
        } catch (error) {
