...
const EFileStatusItem: React.FC<Props> = ({ envelopeId }) => {
  ...
  const { data } = useQuery({
    ...
    retry: false,
    refetchInterval: d =>
      d?.item.filings?.some(f => f.status !== 'submitting')
        ? false
        : 5000 + info.retryCount * 2000,
    onError: () => dispatch({ type: 'INCREMENT_RETRY', envelopeId }),
  });
  ...
  return (
    <li className="mt-2 text-sm text-gray-700" role="status" aria-live="polite">
