...
const EFilePage: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'jurisdiction') {
      // When jurisdiction changes, update county to the first county in that jurisdiction
      setFormData((prev) => ({
        ...prev,
        [name]: value,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show success message (could use a toast or other notification)
      alert('eFiling submitted successfully!');
    } catch (error) {
      console.error('Error submitting form:', error);
      alert('There was an error submitting your eFiling. Please try again.');
    } finally {
              ...
              {formData.files && (
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    {Array.from(formData.files).length} file(s) selected
                  </p>
