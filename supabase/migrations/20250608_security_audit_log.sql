...
BEGIN
  ...
  WHERE blocked_at < now() - (retention_days || ' days')::interval;
  
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  
  RAISE NOTICE 'Cleaned up % old audit log entries (older than % days)', 
...
-- Grant execute to service role only (for scheduled cleanup)
...
DO $$
BEGIN
  RAISE NOTICE 'Security audit system created successfully';
  ...
  RAISE NOTICE '- cleanup_old_audit_logs() function for maintenance';
