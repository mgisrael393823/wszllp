...
BEGIN
  ...
  SELECT id INTO v_existing_user_id FROM auth.users LIMIT 1;
  
  IF v_existing_user_id IS NULL THEN
    RAISE NOTICE 'No users found, skipping anon test';
    RETURN;
  END IF;
  
  -- Insert a test case with valid user_id as service role
  INSERT INTO cases (id, user_id, plaintiff, defendant, address, status)
  VALUES (v_test_id, v_existing_user_id, 'Anon Test', 'Anon Test', 'Test', 'Test');
  ...
  BEGIN
    INSERT INTO cases (id, user_id, plaintiff, defendant, address, status)
    VALUES (gen_random_uuid(), v_existing_user_id, 'Anon Insert', 'Should Fail', 'Test', 'Test');
    RAISE WARNING '❌ ANON CAN INSERT!';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE '✅ Anon INSERT blocked with error: %', SQLERRM;
  END;
  ...
  BEGIN
    ...
    GET DIAGNOSTICS v_anon_update_count = ROW_COUNT;
    IF v_anon_update_count > 0 THEN
      RAISE WARNING '❌ ANON CAN DELETE! Deleted % records', v_anon_update_count;
    ELSE
      RAISE NOTICE '✅ Anon DELETE blocked (0 records affected)';
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE '✅ Anon DELETE blocked with error: %', SQLERRM;
  END;
