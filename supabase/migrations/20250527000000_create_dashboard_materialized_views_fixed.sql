...
-- Fixed version with correct column names (lowercase)

-- First, drop any existing views to start fresh
...
SELECT 
    ...
    COUNT(*) as other_contacts, -- placeholder
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE c.updatedat >= NOW() - INTERVAL '24 hours') as cases_updated_last_24h,
    COUNT(*) FILTER (WHERE c.updatedat >= NOW() - INTERVAL '7 days') as cases_updated_last_7_days,
    ...
    (
        COUNT(*) FILTER (WHERE c.updatedat >= NOW() - INTERVAL '24 hours') +
        (SELECT COUNT(*) FROM public.hearings WHERE updated_at >= NOW() - INTERVAL '24 hours') +
        (SELECT COUNT(*) FROM public.documents WHERE updated_at >= NOW() - INTERVAL '24 hours')
...
SELECT 
    ...
    CASE 
        ...
        THEN ROUND((cs.active_cases::NUMERIC / cs.total_cases) * 100, 1)
        ELSE 0 
    END as active_cases_percentage,
    
    CASE 
        WHEN COALESCE(ds.total_documents, 0) > 0 
        THEN ROUND((ds.pending_documents::NUMERIC / ds.total_documents) * 100, 1)
        ELSE 0 
    END as pending_documents_percentage,
    
    CASE 
        WHEN COALESCE(hs.total_hearings, 0) > 0 
        THEN ROUND((hs.hearings_next_30_days::NUMERIC / hs.total_hearings) * 100, 1)
        ELSE 0 
...
FROM 
    (SELECT 1) AS dummy
    LEFT JOIN dashboard_cases_summary cs ON true
    LEFT JOIN dashboard_hearings_summary hs ON true  
    LEFT JOIN dashboard_documents_summary ds ON true
    LEFT JOIN dashboard_contacts_summary cos ON true
    LEFT JOIN dashboard_activity_summary acts ON true;

-- Create unique index on the combined metrics view
CREATE UNIQUE INDEX IF NOT EXISTS idx_dashboard_combined_metrics_unique 
ON dashboard_combined_metrics (last_refreshed);
...
RETURNS VOID AS $$
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY dashboard_cases_summary;
    REFRESH MATERIALIZED VIEW CONCURRENTLY dashboard_hearings_summary;
    REFRESH MATERIALIZED VIEW CONCURRENTLY dashboard_documents_summary;
    REFRESH MATERIALIZED VIEW CONCURRENTLY dashboard_contacts_summary;
    REFRESH MATERIALIZED VIEW CONCURRENTLY dashboard_activity_summary;
    REFRESH MATERIALIZED VIEW CONCURRENTLY dashboard_combined_metrics;
END;
$$ LANGUAGE plpgsql;
